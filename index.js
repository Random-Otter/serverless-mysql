...
module.exports = (params) => {
  ...
  const decorrelatedJitter = (sleep = 0) =>
    Math.min(cap, randRange(base, sleep * 3));
    ...
    try {
      ...
      if (tooManyConnsErrors.includes(e.code) && retries < maxRetries) {
        ...
        onRetry(
          ...
          retries,
          sleep,
          typeof backoff === "function" ? "custom" : backoff
        ); // fire onRetry event
    ...
    if (client !== null && manageConns) {
      ...
      if (usedConns.total / maxConns.total > connUtilization) {
        ...
        let timeout = Math.min(
          Math.max(usedConns.maxAge, zombieMinTimeout),
          zombieMaxTimeout
        );

        // Kill zombies if they are within the timeout
        let killedZombies =
          timeout <= usedConns.maxAge
            ? await killZombieConnections(timeout)
            : 0;
  ...
  const transaction = () => {
    ...
    return {
      ...
      enqueueFunction: function(fn) {
        if (typeof fn === "function") {
          functionIndices.add(queries.length);
          queries.push(fn);
        }
  ...
  backoff =
    typeof cfg.backoff === "function"
      ? cfg.backoff
      : cfg.backoff &&
        ["full", "decorrelated"].includes(cfg.backoff.toLowerCase())
  ...
  zombieMinTimeout = Number.isInteger(cfg.zombieMinTimeout)
    ? cfg.zombieMinTimeout
    : 3; // default to 3 seconds
  zombieMaxTimeout = Number.isInteger(cfg.zombieMaxTimeout)
    ? cfg.zombieMaxTimeout
    : 60 * 15; // default to 15 minutes
  maxConnsFreq = Number.isInteger(cfg.maxConnsFreq)
    ? cfg.maxConnsFreq
    : 15 * 1000; // default to 15 seconds
  ...
  onKillError =
    typeof cfg.onKillError === "function" ? cfg.onKillError : () => {};

  let connCfg =
    typeof cfg.config === "object" && !Array.isArray(cfg.config)
      ? cfg.config
      : {};
